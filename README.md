<!-- ========================= -->
<!--        HEADER             -->
<!-- ========================= -->

<div align="center">

<img src="https://capsule-render.vercel.app/api?type=waving&color=0:0F2027,50:203A43,100:2C5364&height=220&section=header&text=Engr.%20Saleem%20Abbas&fontSize=45&fontColor=ffffff&animation=fadeIn&fontAlignY=35" width="100%"/>

# 👋 Hi, I'm Engr. Saleem Abbas

### Embedded & Hardware Engineer | Industrial IoT | Robotics | PCB Design

<img src="https://readme-typing-svg.demolab.com?font=Fira+Code&size=20&duration=3000&pause=800&color=2F81F7&center=true&vCenter=true&width=750&lines=Embedded+Systems+Engineer;ESP32+%7C+STM32+%7C+Arduino;RS485+%7C+Modbus+RTU+%7C+UART;PCB+Design+%7C+Hardware+Prototyping;Industrial+IoT+%7C+Smart+Agriculture;ROS+2+%7C+Robotics+%7C+Automation" />

<br>

<a href="https://github.com/SALEEEM-ABBAS">
<img src="https://img.shields.io/github/followers/SALEEEM-ABBAS?label=Followers&style=for-the-badge&logo=github"/>
</a>

<a href="https://github.com/SALEEEM-ABBAS?tab=repositories">
<img src="https://img.shields.io/badge/Repositories-Explore-181717?style=for-the-badge&logo=github"/>
</a>

</div>

---

# 🧑‍💻 About Me

I'm an **Embedded & Hardware Engineer** focused on designing and developing
complete electronic systems — from **circuit design and PCB development**
to **firmware, communication protocols, sensor integration, testing,
and real-world deployment**.

My work sits at the intersection of:

> 🔧 Hardware + 💻 Firmware + 📡 Communication + 🤖 Automation

I enjoy taking an idea from a **schematic or prototype** and turning it
into a reliable working embedded system.

---

# 📫 Connect With Me

<div align="center">

<a href="https://www.linkedin.com/in/abbas-shigree/">
<img src="https://img.shields.io/badge/LinkedIn-Abbas%20Shigree-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white"/>
</a>

<a href="mailto:saleem.shigri.05@gmail.com">
<img src="https://img.shields.io/badge/Email-saleem.shigri.05%40gmail.com-D14836?style=for-the-badge&logo=gmail&logoColor=white"/>
</a>

<a href="https://github.com/SALEEEM-ABBAS">
<img src="https://img.shields.io/badge/GitHub-SALEEEM--ABBAS-181717?style=for-the-badge&logo=github&logoColor=white"/>
</a>

</div>


# ⚡ What I Build

```text
        💡 IDEA
          │
          ▼
    📐 SYSTEM DESIGN
          │
          ▼
    🔌 CIRCUIT / PCB
          │
          ▼
    🔧 PROTOTYPING
          │
          ▼
    💻 FIRMWARE
          │
          ▼
    📡 COMMUNICATION
          │
          ▼
    🧪 TESTING & CALIBRATION
          │
          ▼
    🚀 REAL-WORLD SYSTEM

---

# 🛠️ Technical Skills

### 🔌 Microcontrollers & SBCs

<p>
<img src="https://img.shields.io/badge/ESP32-E7352C?style=for-the-badge&logo=espressif&logoColor=white"/>
<img src="https://img.shields.io/badge/STM32-03234B?style=for-the-badge&logo=stmicroelectronics&logoColor=white"/>
<img src="https://img.shields.io/badge/Arduino-00979D?style=for-the-badge&logo=arduino&logoColor=white"/>
<img src="https://img.shields.io/badge/Raspberry%20Pi-C51A4A?style=for-the-badge&logo=raspberrypi&logoColor=white"/>
<img src="https://img.shields.io/badge/Banana%20Pi-ED1C24?style=for-the-badge"/>
</p>

### 💻 Programming

<p>
<img src="https://img.shields.io/badge/C%2FC%2B%2B-00599C?style=for-the-badge&logo=cplusplus&logoColor=white"/>
<img src="https://img.shields.io/badge/Python-3776AB?style=for-the-badge&logo=python&logoColor=white"/>
<img src="https://img.shields.io/badge/MATLAB-0076A8?style=for-the-badge&logo=mathworks&logoColor=white"/>
</p>

### 📡 Communication Protocols

<p>
<img src="https://img.shields.io/badge/RS485-1F2937?style=for-the-badge"/>
<img src="https://img.shields.io/badge/Modbus%20RTU-0B7285?style=for-the-badge"/>
<img src="https://img.shields.io/badge/UART-374151?style=for-the-badge"/>
<img src="https://img.shields.io/badge/SPI-4B5563?style=for-the-badge"/>
<img src="https://img.shields.io/badge/I2C-6B7280?style=for-the-badge"/>
</p>

### 📐 PCB Design & Simulation

<p>
<img src="https://img.shields.io/badge/KiCad-314CB6?style=for-the-badge&logo=kicad&logoColor=white"/>
<img src="https://img.shields.io/badge/Proteus-1B365D?style=for-the-badge"/>
<img src="https://img.shields.io/badge/LTspice-9B59B6?style=for-the-badge"/>
<img src="https://img.shields.io/badge/DipTrace-222222?style=for-the-badge"/>
</p>

### 🐧 Operating Systems & Tools

<p>
<img src="https://img.shields.io/badge/Linux-FCC624?style=for-the-badge&logo=linux&logoColor=black"/>
<img src="https://img.shields.io/badge/Ubuntu-E95420?style=for-the-badge&logo=ubuntu&logoColor=white"/>
<img src="https://img.shields.io/badge/Windows-0078D6?style=for-the-badge&logo=windows&logoColor=white"/>
<img src="https://img.shields.io/badge/Git-F05032?style=for-the-badge&logo=git&logoColor=white"/>
<img src="https://img.shields.io/badge/GitHub-181717?style=for-the-badge&logo=github&logoColor=white"/>
</p>

---

# 🚀 Featured Engineering Projects

## 🌱 7-in-1 RS485 Soil Sensor

An industrial-style soil monitoring system based on **ESP32 + RS485**.

### Features

* 🌡️ Temperature measurement
* 💧 Soil moisture estimation
* ⚡ Electrical conductivity
* 🧪 Soil pH estimation
* 🌱 NPK estimation
* 📡 RS485 communication
* 🔄 Modbus RTU
* 📊 Sensor calibration and filtering

**Technology:**
`ESP32` `RS485` `Modbus RTU` `ADC` `DS18B20` `Sensors`

---

## 🤖 Agricultural Robotics Platform

A robotics platform focused on autonomous agricultural applications.

### Technologies

* 🤖 ROS 2
* 📡 LiDAR
* 🧭 GNSS / GPS
* 🗺️ SLAM
* ⚙️ Motor Control
* 🔌 ESP32
* 🚜 Agricultural Automation

**Technology:**
`ROS 2` `micro-ROS` `ESP32` `LiDAR` `GNSS` `Robotics`

---

## 📷 ESP32-CAM IoT Vision System

Wi-Fi enabled embedded camera system for remote monitoring and image capture.

### Features

* 📷 JPEG image capture
* 🌐 Web interface
* 📡 Wi-Fi connectivity
* 💡 Flash LED control
* 🖥️ Embedded web server

**Technology:**
`ESP32-CAM` `C++` `Wi-Fi` `Embedded Web Server`

---

## ⚖️ Embedded Weight Measurement System

Microcontroller-based weight measurement system using load-cell instrumentation and digital communication.

**Technology:**
`ESP32` `HX711` `Load Cell` `UART` `Modbus`

---

# 🔬 Engineering Interests

| Area                 | Focus                          |
| -------------------- | ------------------------------ |
| 🔧 Embedded Systems  | ESP32, STM32, Arduino          |
| 📡 Industrial IoT    | RS485, Modbus, Sensors         |
| 🌱 Smart Agriculture | Soil Monitoring & Automation   |
| 🤖 Robotics          | ROS 2, LiDAR, SLAM             |
| 📐 PCB Design        | KiCad, Proteus, LTspice        |
| ⚙️ Automation        | Motor Control & Sensors        |
| 🧪 Sensor Systems    | Calibration & Data Acquisition |
| 🐧 Embedded Linux    | Ubuntu, SBCs                   |

---

# 🧠 Currently Learning

### ROS 2 & Robotics

* ROS 2 architecture
* micro-ROS
* Navigation
* SLAM
* Robotics integration

### Embedded Linux

* SBC deployment
* Linux hardware integration
* System configuration
* Embedded development environments

### Advanced Embedded Systems

* Industrial communication
* Sensor calibration
* Reliable firmware architecture
* IoT system development

---

# 🧩 My Engineering Approach

I don't see hardware and software as separate worlds.

```text
             COMPLETE SYSTEM
                    │
          ┌─────────┴─────────┐
          │                   │
       HARDWARE           SOFTWARE
          │                   │
       Sensors             Firmware
       PCB                  Drivers
       Power                Protocols
       Motors               Data
          │                   │
          └─────────┬─────────┘
                    │
                    ▼
              WORKING PRODUCT
```

My goal is to build systems that are **functional, testable, maintainable, and ready for real-world applications.**

---

# 📊 GitHub Statistics

<div align="center">

<img src="https://github-readme-stats.vercel.app/api?username=SALEEEM-ABBAS&show_icons=true&theme=tokyonight&hide_border=true&rank_icon=github" height="180"/>

<img src="https://github-readme-stats.vercel.app/api/top-langs/?username=SALEEEM-ABBAS&layout=compact&theme=tokyonight&hide_border=true" height="180"/>

</div>

---

# 🔥 GitHub Streak

<div align="center">

<img src="https://github-readme-streak-stats.herokuapp.com/?user=SALEEEM-ABBAS&theme=tokyonight&hide_border=true"/>

</div>

---

# 🎯 2026 Engineering Goals

* 🔥 Build production-oriented embedded systems
* 📡 Develop reliable RS485 / Modbus devices
* 🌱 Expand smart agriculture automation
* 🤖 Develop ROS 2 robotic systems
* 🧠 Improve embedded Linux expertise
* 🔌 Design custom PCBs
* 🚀 Convert prototypes into deployable products

---

# 💬 Ask Me About

`ESP32` • `STM32` • `Arduino` • `C/C++` • `Python`

`RS485` • `Modbus RTU` • `UART` • `SPI` • `I2C`

`PCB Design` • `KiCad` • `Proteus` • `Sensors`

`Load Cells` • `Motor Control` • `Industrial IoT`

`Smart Agriculture` • `ROS 2` • `Robotics` • `Linux`

---

# 🤝 Connect With Me

<div align="center">

<a href="https://www.linkedin.com/in/abbas-shigree/">
<img src="https://img.shields.io/badge/LinkedIn-Abbas%20Shigree-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white"/>
</a>

<a href="mailto:saleem.shigri.05@gmail.com">
<img src="https://img.shields.io/badge/Email-saleem.shigri.05%40gmail.com-D14836?style=for-the-badge&logo=gmail&logoColor=white"/>
</a>

<a href="https://github.com/SALEEEM-ABBAS">
<img src="https://img.shields.io/badge/GitHub-SALEEEM--ABBAS-181717?style=for-the-badge&logo=github&logoColor=white"/>
</a>

</div>

---

<div align="center">

### ⚡ Build Hardware. Write Firmware. Solve Real Problems.

</div>


